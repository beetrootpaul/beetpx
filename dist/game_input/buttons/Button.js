...
class Button {
    ...
    static setRepeatingParamsFor(updateFps) {
        __classPrivateFieldSet(_a, _a, updateFps === 30
            ...
            : updateFps === 60
                ? 30
                : Utils_1.BpxUtils.throwError(`Unsupported desiredUpdateFps: ${updateFps}`), "f", _Button_repeatingFramesStart);
        __classPrivateFieldSet(_a, _a, updateFps === 30
            ? 4
            : updateFps === 60
                ? 8
                : Utils_1.BpxUtils.throwError(`Unsupported desiredUpdateFps: ${updateFps}`), "f", _Button_repeatingFramesInterval);
    }
    get isPressed() {
    ...
    update(isPressed) {
        ...
        if (isPressed && __classPrivateFieldGet(this, _Button_repeatingTimer, "f")?.hasFinished) {
            __classPrivateFieldSet(this, _Button_repeatingTimer, new Timer_1.BpxTimer({
